...
def _fake_server(t):
    ...
    state = {"blob": "", "seen": [], "email": "", "password": ""}
    ...
    def handle(route):
        ...
        if path in ("register", "login"):
            if path == "register":
                state["email"], state["password"] = body.get("email", ""), body.get("password", "")
            elif body.get("password") != state["password"]:
...
def run(t):
    ...
    t.page.locator("#acct-pw").fill("a long enough password")
    ...
    t.check("the password is not written down anywhere",
            any("a long enough password" in v for v in stored.values()), False)
